...
class Model(PseudoSpectralKernel):
    ...
    def _initialize_filter(self):
        ...
        cphi=0.46*0.5*(np.sqrt(self.L**2.+self.W**2.)) #SALAH changed from cphi=0.65*pi so that it works for arbitrary dimensions
        # it means there will be damping on the last 35% of wavenumbers (by which I mean if you look at k_x, first 65% undamped but 
        # on the edge of this range (k_x65%, 10) will be a damped wavevector) 
        wvx=np.sqrt((self.k*self.dx)**2.+(self.l*self.dy)**2.)
        filtr = np.exp(-self.filterfac*(wvx-cphi)**4.)
        filtr[wvx<=cphi] = 1.
        
        #SALAH large scale energy removal#
        cutoffwn=4
        cpsi=np.sqrt((cutoffwn*self.dx)**2.+(cutoffwn*self.dy)**2.)
        filtr[wvx<=cpsi]=np.exp(-self.dragf*(cpsi-wvx)**4.)[wvx<=cpsi]
        
        #SALAH for setting the spectrum we take out the energy in the place we force
        lowb = self.fk1
        upb = self.fk2
        clow = np.sqrt((lowb*self.dx)**2.+(lowb*self.dy)**2.)
        cup = np.sqrt((upb*self.dx)**2.+(upb*self.dy)**2.)
        filtr[np.logical_and(wvx<cup,wvx>clow)]=0
    ...
    def _do_external_forcing(self):
        ...
        #make the direction of forcing do brownian motion
        #the actual forcing is applied in the tendency in the cython code
        #the forcing range etc is defined in the model specific file
        #### Salah ####
        # making forcing AR1
        sz = self.forcerand.shape
        self.forcerand = np.exp(2*np.pi*1j*self.rng.random((sz[0],sz[1]))/100)*self.forcerand
