...
# see if we got a compile time flag (hardcoded to 1 to avoid error)
...
#### SALAH ####
# The wisdom file ensures fft done the same way each time, this ensures bit reproducibility
# At around line 230 another bit of code to load up the wisdom. 
...
cdef class PseudoSpectralKernel:
    ...
    # spectral forcing
    # TODO: figure out if this really needs to be public
    cdef public DTYPE_com_t [:, :] force
    cdef public DTYPE_com_t [:, :] forcerand
    cdef public DTYPE_com_t forceamp
    ...
    def __init__(self, int nz, int ny, int nx, int fftw_num_threads=1): 
        ...
        IF PYQG_USE_PYFFTW:
            ...
            try:
                ...
                print("Wisdom not saved as old wisdom found")
            #### SALAH END ####
            ...
            for j in prange(self.nl, nogil=True, schedule='static',
                ...
                for i in range(self.nk):
                    qh_new[k,j,i] = self.filtr[j,i] * (
                        self.qh[k,j,i] +
                        dt1 * self.dqhdt[k,j,i] + # worried that mixing AB3 and newton raphson is not working - fine if tscale f long
                        dt2 * self.dqhdt_p[k,j,i] +
                        dt3 * self.dqhdt_pp[k,j,i]
                    #+ self.dt*self.forcerand[j,i]*self.force[j,i] # SALAH 
                    ) + self.forcerand[j,i]*self.force[j,i] # config for just setting the spectrum rather than forcing 
